											...
											uint16_t       USART_GPIO_TX_PIN)
{
	...
	usart->USART_GPIO_RX_PIN=USART_GPIO_RX_PIN;
	usart->USART_GPIO_RX_PORT=USART_GPIO_RX_PORT;
	usart->USART_GPIO_TX_PIN=USART_GPIO_TX_PIN;
	usart->USART_GPIO_TX_PORT=USART_GPIO_TX_PORT;
...
{
 HAL_UARTEx_ReceiveToIdle_DMA(huart,DataBuff,BUF_SIZE);//BUF_SIZE 此处填写一次接收的最大数据长度
